...
class TinyCNN(Classifier):
    def __init__(self,
        ...
        # Store class list and input dimensions
        self.classes = classes
        self.height = height
        self.length = length

        # Initialize base Classifier (handles saving, loading, etc.)
        ...
        # After conv + pool, spatial dims: 160→80 (half), then flatten
        h_out = self.height // 2
        l_out = self.length // 2
        flat_feats = 8 * h_out * l_out

        # Final fully‐connected layer: flat_feats → number of classes
